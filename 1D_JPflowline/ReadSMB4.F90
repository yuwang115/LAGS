...
FUNCTION ReadSMB(Model,nodenumber,VarIn)  RESULT(VarOut)
  ...
  INTEGER,PARAMETER           :: FileLength = 104, FileUnit = 39
  CHARACTER(len=MAX_NAME_LEN) :: FileName='smb_AM04AM01_timestep.asc'
...
!  REAL(KIND=dp),SAVE          :: TimeOffset  
  ...
  REAL(kind=dp),ALLOCATABLE,SAVE :: smb(:)
...
!  dt = Model % Solver % dt
  ...
  IF (FirstTime) THEN
!    TimeOffset = tt - dt
    ALLOCATE(smb(FileLength))
    OPEN(UNIT=FileUnit, FILE=FileName)
    DO ii = 1,FileLength
      READ(FileUnit,*) smb(ii)
    END DO
    CLOSE(UNIT=FileUnit)
    FirstTime = .FALSE.
  END IF
